...
output: rmarkdown::html_vignette
  ...
  #   toc: yes
vignette: >
  %\VignetteIndexEntry{SCOUTer demo_update_v.1} 
...
```{r setup}
fun.dir <- "C:/Users/albag/Alba/Doctorado/Investigación/SCOUTER/Update_1/SCOUTerRpack_update/R/"
f.names <- dir(fun.dir)
for (funname in f.names[grepl(".R$", f.names)]){
  source(paste0(fun.dir, funname))
} 
load("C:/Users/albag/Alba/Doctorado/Investigación/SCOUTER/Update_1/SCOUTerRpack_update/data/X.rda")
```
...
```{r scout_x_steps_PC1, fig.cap="Distance plot (left) and score plot (right) of the data simulated with steps mode.", fig.dim = c(6, 3)}
x.out.steps <- scout(x, pcamodel_ref, T2.y = 40, SPE.y = 40, nsteps = 10, mode = "steps", A = 1)
x.all <- rbind(x, x.out.steps$X)
tag.all <- dotag(x, x.out.steps$X)
dscplot(x.all, pcamodel_ref, obstag = tag.all)
```

```{r scout_x_steps_PC2, fig.cap="Distance plot (left) and score plot (right) of the data simulated with steps mode.", fig.dim = c(6, 3)}
x.out.steps <- scout(x, pcamodel_ref, T2.y = 40, SPE.y = 40, nsteps = 10, mode = "steps", A = 2)
x.all <- rbind(x, x.out.steps$X)
tag.all <- dotag(x, x.out.steps$X)
dscplot(x.all, pcamodel_ref, obstag = tag.all)
```
